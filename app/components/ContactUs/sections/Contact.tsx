...
const Contact: React.FC<PlatformsSectionProps> = ({ data, title }) => {
  ...
  return (
    ...
    <section className=" ">
      <div className="container">
          <div className=" py-[20px] md:py-[70px] xl:py-[140px]  border-b border-[#1E1E1E66] ">
          <div >
            {data.map((item, index) => (
              <div key={index} className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-3 justify-center   items-center">
                  {item.details.map((detail, detailIndex) => (
                         <div key={detailIndex} className="py-[25px] text-center lg:border-r border-[#1E1E1E40] h-full last:border-r-0 flex flex-col justify-center items-center">
