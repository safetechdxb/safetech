...
const CarouselTypeTwoItem = ({ image, imageAlt, title, description,isActive,hideDesc }: CarouselTypeTwoItemProps) => {
  ...
  return (
    <div onMouseEnter={() => setIsHovered(true)}
      ...
      >
        <p className={`text-20 font-normal leading-[1.5] group-hover:opacity-100 ${isActive && !hideDesc ? 'opacity-100' : 'opacity-0'} overflow-hidden transition-all duration-700`}>{description}</p>
      </div>
    </div>
  );
}
