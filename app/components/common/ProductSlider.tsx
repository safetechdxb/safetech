    ...
    return(
      <div className="container overflow-visible">
        <div className="flex flex-wrap align-center justify-between gap-y-4 lg:gap-0 mb-4 md:mb-8 2xl:mb-20">
          ...
          {/* Custom Nav Buttons */}
          <div className="flex" >
            <button ref={prevRef} disabled={isBeginning} className={`text-secondary font-bold border-2 border-secondary first:border-r-0 px-8 py-4 h-fit group hover:bg-secondary hover:text-white transition ${isBeginning ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
            ...
            <button ref={nextRef} disabled={isEnd} className={`text-secondary font-bold border-2 border-secondary first:border-r-0 px-8 py-4 h-fit group hover:bg-secondary hover:text-white transition ${isEnd ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
        ...
        <div>
          <Swiper className="!overflow-visible"
            ...
            onSlideChange={(swiper) => {
              ...
              console.log(swiper.activeIndex,swiper.slides.length,swiper.params.slidesPerView)
              const isAtEnd = swiper.activeIndex === swiper.slides.length - (Math.floor(swiper.params?.slidesPerView as number));
              setIsEnd(isAtEnd);
            }}
            onSwiper={(swiper) => {
              setIsBeginning(swiper.isBeginning);
