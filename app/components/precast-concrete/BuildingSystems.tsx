        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
          {
            data.thirdSection.items.map((item, index) => (
              <div className='group p-5 lg:p-8 bg-white hover:bg-primary trasition-all duration-300 hover:-translate-y-2' key={index}>
                <div className='border-b border-secondary/40 pb-10 group-hover:border-white trasition-all duration-300'>
                  <div className="bg-primary p-3 w-20 h-20 group-hover:bg-white trasition-all duration-300">
                    <Image src={item.logo} alt={item.logoAlt} width={100} height={100} className='w-full h-auto object-contain brightness-0 invert-100 group-hover:brightness-100 group-hover:invert-0' />
                  </div>
                </div>
